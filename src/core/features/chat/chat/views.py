...
# Custom Pagination for Chat Messages with diagnostic logging
class ChatMessagePagination(pagination.PageNumberPagination):
    page_size = 50
    page_size_query_param = 'limit'
    ...
    def get_paginated_response(self, data, participants_data):
        ...
        print(f"--- CHAT DEBUG: Final paginated response data being sent: {response_data} ---")
...
class ChatMessageListView(APIView):
    ...
    def get(self, request, room_id):
        print(f"--- CHAT DEBUG: ENTERING ChatMessageListView for room {room_id} ---")
        room = get_object_or_404(ChatRoom, id=room_id)
        ...
        print(f"--- CHAT DEBUG: Participant objects: {participants} ---")
        
        participants_serializer = SimpleUserSerializer(participants, many=True)
        print(f"--- CHAT DEBUG: Serialized participants: {participants_serializer.data} ---")
        ...
        # This part is a fallback, might not be hit with pagination
        print(f"--- CHAT DEBUG: Non-paginated response for room {room_id} ---")
...
class ChatRoomListView(APIView):
    ...
    def get(self, request):
        print("--- CHAT DEBUG: ENTERING ChatRoomListView ---")
        user = request.user if request.user.is_authenticated else None
        if not user:
        ...
        print(f"--- CHAT DEBUG: Result for ChatRoomList before wrapping: {result} ---")
        response = Response({"items": result})
        print(f"--- CHAT DEBUG: Final response for ChatRoomList being sent: {response.data} ---")
        return response
